defmodule IntentLedger.TelemetryTest do
  ...
  test "inspection APIs emit inspection telemetry" do
    ledger = unique_atom(:inspection_telemetry_ledger)
    prefix = [unique_atom(:inspection_telemetry_prefix)]
    inspection_event = Telemetry.event_name(:inspection_stop, telemetry_prefix: prefix)
    handler_id = attach_events([inspection_event])

    start_supervised!(
      {IntentLedger,
       name: ledger, store: IntentLedger.Store.Memory, queues: [default: [shards: 1]], telemetry_prefix: prefix}
    )

    assert {:ok, _record} =
             IntentLedger.submit(ledger, %{key: "job:inspection-telemetry", kind: "job.run"},
               command_id: "cmd_inspection_telemetry_submit"
             )

    try do
      assert {:ok, queues} = IntentLedger.inspect_queues(ledger)

      assert_receive {:telemetry, ^inspection_event, %{duration: duration, count: count}, metadata}
      assert is_integer(duration)
      assert duration >= 0
      assert count == length(queues)
      assert metadata.ledger == ledger
      assert metadata.operation == :queues
      assert metadata.status == :ok
